"""Panel crew orchestration entrypoint.
...
from panelcore.tasks import TASKS
...
def run(config: PanelConfig | None = None) -> Path:
    """Run a Panel review and return the path to the report file.
    ...
    report_path.write_text(
        "# Panel Report (stub)\n\n"
        "Panel crew is not fully wired yet. Use agent skills for real reviews.\n\n"
        ...
        "and meet Approves from `panelcore/permissions.py` / `COLLABORATION.md`.\n\n"
        "Until the Python crew is complete, use `PANEL.md` with Cursor / Claude Code:\n\n"
        "> Run a panel\n",
        encoding="utf-8",
    )
    return report_path
