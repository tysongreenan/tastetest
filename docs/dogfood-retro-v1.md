# Dogfood retro — why TasteTest failed its first self-review
...
2. **Fix (pass 1):** Applied hard gates aggressively. Homepage got “purer” but **lost the obvious install path** (`npx tastetest init` copy) and **gutted SmoothUI usage** that the product had intentionally adopted.
...
## What the product (EmpathFlow skill) did wrong
...
EmpathFlow is supposed to help *this* product feel fluid. Pass-1 rewrote TasteTest’s story (skill-only prose) instead of polishing the story in the README/landing copy (CLI import into the repo).

### 3. Incomplete implementation ≠ bad UX pattern
`npx tastetest init` not shipping yet is an **implementation gap**. Copy-to-install is still the **right convention** for dev tools. The correct move: keep the pattern, wire `init`, or label roadmap — not hide the pattern.
...
| Guardrail | Where |
|-----------|--------|
| Preserve-first rule + fix order | `EMPATHFLOW.md` |
| Failure-mode table (over-correction, purity over intent) | `EMPATHFLOW.md` |
| Intent & constraints intake | `EMPATHFLOW.md` process step 1 |
| Fix guardrails + definition of done | `EMPATHFLOW.md` |
| Anti-patterns for reviewers | `playbook.md` |
| Gates diagnose, don’t authorize deletion | `playbook.md` + `EMPATHFLOW.md` |

---

## How to use this in future runs

When EmpathFlow recommends a fix:
